...
void main() {
    ...
    for(int i = 0; i < 10; i++) {
        ...
        vec3 light_vector = normalize(light_position[i] - pos);
        vec3 normalized = normalize(vertex_normal);
        diffuse += max(light_color[i] * max(dot(normalized, light_vector), 0.0), 0.0);
        
        vec3 view_vector = normalize(camera_position - pos);
        vec3 reflection_vector = normalize(reflect(-light_vector, normalized));
        specular += max(light_color[i] * pow(max(dot(reflection_vector, view_vector), 0.0), material_shininess), 0.0);
    }
}
